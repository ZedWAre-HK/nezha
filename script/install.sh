#!/bin/sh
# ZedWAre-HK/nezha v0-final installer with Cloudflare Access login support.

NEZHA_REPOSITORY="ZedWAre-HK/nezha"
NEZHA_BRANCH="v0-final"
NEZHA_DASHBOARD_VERSION="v0.20.13-cloudflare.4"
...
modify_dashboard_config() {
    ...
    echo "OAuth2 Callback 地址为 http(s)://域名或IP/oauth2/callback"
        echo "Cloudflare Access 请创建 SaaS OIDC 应用，并启用 openid、email、profile scopes。"
        printf "请输入 OAuth2 提供商(github/gitlab/jihulab/gitee/gitea/cloudflare，默认 github): "
        ...
        printf "请输入管理员登录名（Cloudflare 填邮箱），多个以逗号隔开: "
        read -r nz_admin_logins
        printf "请输入 OAuth2 Endpoint（Cloudflare 填 https://<team>.cloudflareaccess.com，其他类型可留空）: "
...
update_docker_compose_image() {
    ...
    if grep -q "registry.cn-shanghai.aliyuncs.com/naibahq/nezha-dashboard" "$yaml_file_path"; then
        sed -i "s|registry.cn-shanghai.aliyuncs.com/naibahq/nezha-dashboard[^[:space:]]*|ghcr.io/zedware-hk/nezha-dashboard:${NEZHA_DASHBOARD_VERSION}|" "$yaml_file_path"
    fi
...
restart_and_update_standalone() {
    ...
    if [ -z "$CN" ]; then
        NZ_DASHBOARD_URL="https://${GITHUB_URL}/${NEZHA_REPOSITORY}/releases/download/${_version}/dashboard-linux-${os_arch}.zip"
    else
        NZ_DASHBOARD_URL="https://${GITHUB_URL}/${NEZHA_REPOSITORY}/releases/download/${_version}/dashboard-linux-${os_arch}.zip"
    fi
...
uninstall_dashboard_docker() {
    ...
    sudo docker rmi -f "ghcr.io/zedware-hk/nezha-dashboard:${NEZHA_DASHBOARD_VERSION}" >/dev/null 2>&1
}
...
show_menu() {
    ...
    --- https://github.com/${NEZHA_REPOSITORY} ---
